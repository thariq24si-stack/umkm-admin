@extends('layouts.admin.app') @section('content')
    <div class="py-4">
        ...
        <div class="d-flex justify-content-between w-100 flex-wrap">
            <div class="mb-3 mb-lg-0">
                ...
                <p class="mb-0">List seluruh unit usaha warga</p>
            </div>
            <div>
                <a href="{{ route('umkm.create') }}" class="btn btn-success text-white">
                                ...
                                <div class="col-md-3">
                                    <div class="input-group">
                                        <input type="text" name="search" class="form-control" 
                                            value="{{ request('search') }}" placeholder="Cari Nama Usaha..." aria-label="Search">
    ...
    @foreach ($umkms as $item)
        <tr>
            ...
            <td><span class="fw-bold">{{ $item->nama_usaha }}</span></td>
            <td>{{ $item->pemilik->first_name ?? 'N/A' }} {{ $item->pemilik->last_name ?? '' }}</td>
            <td>{{ $item->kategori }}</td>
            <td>RT {{ $item->rt }} / RW {{ $item->rw }}</td>
            <td>{{ $item->kontak }}</td>
            <td>
    @if($item->logo)
        <img src="{{ asset('assets-admin/img/logo-umkm/' . $item->logo) }}" 
    ...
    @endif
            ...
            <td class="text-center">
                ...
                <a href="{{ route('umkm.edit', $item->umkm_id) }}" class="btn btn-info btn-sm">
                ...
                <form action="{{ route('umkm.destroy', $item->umkm_id) }}" method="POST" style="display:inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Hapus usaha ini?')">
                        <svg class="icon icon-xs" data-slot="icon" fill="none"
                            stroke-width="1.5" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round"
